...
Description:
The code maps videos to predefined clusters as per the hierarchal.py

Inputs:
- Comments data (CSV obtained from public YouTube API).
- Performance metrics file (Created by the video_profiles.py).

Outputs:
- Cached centroids and video-to-cluster mappings.
- A set of recommended videos for each user.
"""

def find_similar_videos(keywords, tfidf_vectorizer, video_tfidf, videos_df, top_n=10):
    """
    Finds and returns a list of videos that have similar keywords to the input query using cosine similarity.
    """
...
def generate_recommendations(user_clusters, tfidf_vectorizer_videos, video_tfidf, user_profiles_df, videos_df):
    """
    Generates recommendations for users based on their profile keywords and clustered user interactions.
    Maps videos to both an outer cluster and a user-specific inner cluster.
    """
    recommendations = {}
    video_cluster_mapping = {} 
    ...
    for _, cluster_row in user_clusters.iterrows():
        ...
        # Dynamically generate an outer cluster ID 
...
def get_cluster_centroids(tfidf_vectorizer_videos, video_tfidf, user_profiles_df, recommendations, videos_df):
    """
    Calculates centroids for both outer clusters using KMeans and inner clusters based on user-specific recommendations.
    """
    kmeans_outer = KMeans(n_clusters=5) 
