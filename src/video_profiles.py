...
Description:
The code processes YouTube video performance data to generate video profiles that are required for further processing of recommendations. 
It calculates key metrics such as net likes gained, net subscribers gained, average view percentage, and a composite 'Popularity Factor' for each video which are key factors to determine a video profile. 
Additionally, it identifies the top country per video based on views. 
The final processed data is saved to a CSV file for further analysis or reporting.

Inputs:
- Aggregated metrics by country and subscriber status (CSV obtained from public YouTube API)
- Aggregated metrics by video (CSV obtained from public YouTube API)
- Comments data (CSV obtained from public YouTube API)
- Video performance over time (CSV obtained from public YouTube API)

Outputs:
- A CSV file containing the video profile data
"""
