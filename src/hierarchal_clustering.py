...
Description:
The code creates hierarchical clusters of videos and users from the previously created video and user profiles 
based on their interaction patterns and content similarity. 
It then calculates centroids for these clusters using TF-IDF vectorization, which is used further to generate recommendations.
...
Outputs:
- A pickle file containing the hierarchical clusters of videos and users.
- A pickle file containing the calculated centroids for these clusters.
"""
...
def find_similar_videos(target_keywords, videos_df, tfidf_vectorizer, top_n=10):
   """
    Finds videos similar to the target video based on the keywords.
    Uses TF-IDF and cosine similarity to find the top_n similar videos.
   """
...
def form_outer_clusters(user_groups, videos_df, user_profiles_df):
    """
    Forms outer clusters of videos based on user interactions and refines them for each user.
    Creates a hierarchical structure with outer clusters containing videos and inner clusters personalized for each user.
    """
...
def calculate_and_save_centroids_to_redis(videos_df, clusters):
    """
    Calculates the centroids of both outer and inner clusters and saves them.
    """
...
def print_cluster_hierarchy(clusters):
    """
    Prints the hierarchy of clusters, showing outer and inner clusters with their associated videos.
    """
